...
from xnornet.binary_ops import xnorize

from xnornet.binary_layers import BinaryDense, BinaryConv2D
...
class XnorConv2D(BinaryConv2D):
    ...
    def call(self, inputs):
        ...
        if self.data_format == 'channels_first':
            outputs = outputs * K.stop_gradient(inputs_a) * K.expand_dims(K.expand_dims(K.expand_dims(kernel_a, 0), -1),
                                                                          -1)
        else:
            outputs = outputs * K.stop_gradient(inputs_a) * K.expand_dims(K.expand_dims(K.expand_dims(kernel_a, 0), 0),
                                                                          0)
